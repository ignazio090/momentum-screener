...
Koersdata via yfinance (gratis, geen key nodig).
Nieuwsartikelen via Finnhub (gratis tier).
Telegram-bericht + dashboard + Excel.
...
# ============================================================
# KOERSDATA (yfinance — gratis, geen key nodig)
# ============================================================
...
# ============================================================
def run_real():
    # Snelle test: kan yfinance data ophalen?
    print("Test yfinance met AAPL...")
    try:
        test = yf.Ticker("AAPL")
        th = test.history(period="5d")
        if th.empty:
            print("yfinance gaf geen data terug. Mogelijk een tijdelijk probleem.")
            sys.exit(1)
        print("yfinance werkt! AAPL slotkoers: $" + "%.2f" % float(th["Close"].iloc[-1]))
    except Exception as e:
        print("yfinance test mislukt: " + str(e))
        sys.exit(1)
    ...
    for i, tk in enumerate(universe, 1):
        ...
        if ch is None:
            errors += 1
            if errors <= 3:
                print("  x " + tk + ": geen data")
        elif is_hit(ch):
            news = get_news(tk)
            time.sleep(0.3)  # kleine pauze voor Finnhub news
